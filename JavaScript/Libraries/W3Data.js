...
function w3DisplayData(id, data) {
    ...
    arr = w3GetElementsByAttribute(html, "vm-repeat");
    l = arr.length;
    for (j = (l - 1) ; j >= 0; j -= 1) {
        cc = arr[j].getAttribute("vm-repeat").split(" ");
        if (cc.length == 1) {
            repeat = cc[0];
        } else {
            repeatX = cc[0];
        ...
        arr[j].removeAttribute("vm-repeat");
        ...
        if (repeatObj && typeof repeatObj == "object" && repeatObj.length != "undefined") {
            ...
            console.log("vm-repeat must be an array. " + repeat + " is not an array.");
            continue;
        }
    }
...
function w3IncludeHTML() {
    ...
    for (i = 0; i < z.length; i++) {
        if (z[i].getAttribute("vm-include-html")) {
            a = z[i].cloneNode(false);
            file = z[i].getAttribute("vm-include-html");
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (xhttp.readyState == 4 && xhttp.status == 200) {
                    a.removeAttribute("vm-include-html");
