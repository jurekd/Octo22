...
function WordSearchGameInit (Settings,taskContent){
  'use strict';
  ...
  function WordSearch(wrapEl, settings) {
    ...
    //   'gridSize': 10,
    //   'words': ['ptolemeusz', 'liczbapi', 'priorytet', 'kodowanie', 'wzór'],
	  // 'wordsList' : [],
    //   'debug': false
    // }
  ...
  WordSearch.prototype.lookup = function(selected) {
    ...
    if (this.settings.words.indexOf(words[0]) > -1 ||
      ...
      if (document.querySelector('.ws-question') !== null) {
          var questions = document.getElementsByClassName("ws-question");
          for(var i = 0;i<questions.length;i++){
            if(questions[i].getAttribute('data-ws-word') && questions[i].getAttribute('data-ws-word').toUpperCase() == words[0] ){
              questions[i].innerHTML =  "<del>"+questions[i].innerHTML+"</del>";
            }
          }
      }
